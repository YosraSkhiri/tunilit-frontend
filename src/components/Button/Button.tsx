...
const Button = ({
	...
	const buttonClass = classNames(
      ...
      [styles['btn--disabled-outlined']]: disabled || loading && variant !== "subtle",
			[styles['btn--disabled-subtle']]: disabled || loading && variant === "subtle",
		},
		className
	)
	...
	const WrapperElem: WrapperElemType =
		(shadow || arrow) ? 'div' : Fragment
	...
	return (
			...
			</ButtonBase>
      {
        arrow && variant !== 'subtle' ?
          <svg 
            ref={arrowSvgRef}
            className={`${styles.arrow} ${disabled ? styles['arrow--disabled'] : ''}`}
            width="72" 
            height="17" 
            viewBox="0 0 72 17" 
            fill="none" 
